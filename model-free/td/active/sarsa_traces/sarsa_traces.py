...
def main():
    ...
    action_len = env.nA
    # state-action values
    state_action_values = np.zeros((action_len, state_len)).astype('float64')
    policy = np.random.randint(low=0, high=action_len, size=state_len, dtype="int64")
    ## trace matrix
    # trace matrix
    trace_matrix = np.full((action_len, state_len), 1)
    ## initialize episodes
    tot_epoch = 500000
    last_value_change = 0
    ...
    for epoch in range(tot_epoch + 1):
        ...
        while True:
            ...
            # get delta
            new_state_new_action_val = state_action_values[policy[new_observation], new_observation]
            state_action_val = state_action_values[action, observation]
            delta = reward + gamma * (new_state_new_action_val - state_action_val)
            # add one to the state-action visited
            trace_matrix[action, observation] += 1
            # update the utility matrix
            state_action_values = update_utility(state_action_values, trace_matrix, alpha, delta)
            # update trace eligibility matrix
